...
### Density fluctuations: linear fit on log(x) and log(y) ###
        ...
        temp = pd.read_csv(inputs[i], delim_whitespace=True, usecols=[1,2], names=["x","y"], header=None)
        x = np.array(temp.x.tolist(), dtype=float)
        y = np.array(temp.y.tolist(), dtype=float)
